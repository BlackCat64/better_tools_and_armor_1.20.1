...
public record EnergyVialToggleKeyMessage(int eventType, int pressedms) implements CustomPacketPayload {
	...
	}, (RegistryFriendlyByteBuf buffer) -> new EnergyVialToggleKeyMessage(buffer.readInt(), buffer.readInt()));

	@Override
	public Type<EnergyVialToggleKeyMessage> type() {
		return TYPE;
	}

	public static void handleData(final EnergyVialToggleKeyMessage message, final IPayloadContext context) {
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		BetterToolsMod.addNetworkMessage(EnergyVialToggleKeyMessage.TYPE, EnergyVialToggleKeyMessage.STREAM_CODEC, EnergyVialToggleKeyMessage::handleData);
	}
}