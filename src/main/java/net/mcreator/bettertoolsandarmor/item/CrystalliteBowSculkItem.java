...
public class CrystalliteBowSculkItem extends BowItem {
	public CrystalliteBowSculkItem() {
		super(new Item.Properties().durability(3000).fireResistant().rarity(Rarity.COMMON));
	}
	...
	public void appendHoverText(ItemStack itemstack, Level level, List<Component> list, TooltipFlag flag) {
		...
		list.add(Component.literal("\u00A73Resonance - Arrows pierce through all targets"));
	}
			...
			if (!itemstack.isEmpty() || flag) {
				...
				if (!((double) f < 0.1D)) {
					...
					if (!p_40668_.isClientSide) {
						...
						abstractarrow.setPierceLevel((byte) Byte.MAX_VALUE); // Crystallite Sculk effect - Set pierce level to the maximum 
