...
public class DoubleJumpPotion2ReverseRecipeBrewingRecipe implements IBrewingRecipe {
	...
	public static void init(RegisterBrewingRecipesEvent event) {
		event.getBuilder().addRecipe(new DoubleJumpPotion2ReverseRecipeBrewingRecipe());
	}
	...
	public boolean isInput(ItemStack input) {
		...
		return (inputItem == Items.POTION || inputItem == Items.SPLASH_POTION || inputItem == Items.LINGERING_POTION) && optionalPotion.isPresent() && optionalPotion.get().is(BetterToolsModPotions.DOUBLE_JUMP_POTION_2);
	}
	...
	public ItemStack getOutput(ItemStack input, ItemStack ingredient) {
		if (isInput(input) && isIngredient(ingredient)) {
			return PotionContents.createItemStack(input.getItem(), BetterToolsModPotions.DOUBLE_JUMP_POTION);
		}
