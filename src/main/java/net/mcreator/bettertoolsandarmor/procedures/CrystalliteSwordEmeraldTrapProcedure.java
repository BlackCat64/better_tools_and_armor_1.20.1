...
public class CrystalliteSwordEmeraldTrapProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity, boolean isvanillacritical) {
		if (entity == null || sourceentity == null)
			return;
		double speed = 0;
		if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).is(ItemTags.create(new ResourceLocation("better_tools:pitfall_weapons"))) && entity instanceof LivingEntity
				&& (entity instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) > 0 && entity.getPersistentData().getDouble("trapped_ticks") <= 0) {
			if (entity.onGround() && isvanillacritical) {
					...
					if (!_ent.level().isClientSide() && _ent.getServer() != null) {
						_ent.getServer().getCommands().performPrefixedCommand(
								...
								("summon block_display ~ ~ ~ {transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],scale:[" + ("" + entity.getBbWidth() * 1.25) + "f,0.5f," + ("" + entity.getBbWidth() * 1.25) + "f],translation:[-"
										+ ("" + entity.getBbWidth() * 0.625) + "f,0f,-" + ("" + entity.getBbWidth() * 0.625) + "f]},block_state:{Name:mud},ForgeData:{trapped_in_ground:1b}}"));
					}
				}
