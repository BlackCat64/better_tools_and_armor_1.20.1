...
public class CrystalliteSculkArmorReducedDetectionRangeProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (BetterToolsModVariables.MapVariables.get(world).stealth_armor_timer == 0) {
			...
			if (IsWearingGlassArmorFullSetProcedure.execute(entity) && PlayerHasEnergyVialEquippedProcedure.execute(world, entity) && EnergyVialActiveArmorPiecesProcedure.execute(entity, GetEquippedVialProcedure.execute(world, entity)) == 4
					&& EnergyVialActiveProcedure.execute(world, entity, GetEquippedVialProcedure.execute(world, entity)) == 1) {
				range_multiplier = 0;
				armor_pieces = 4;
			} else {
