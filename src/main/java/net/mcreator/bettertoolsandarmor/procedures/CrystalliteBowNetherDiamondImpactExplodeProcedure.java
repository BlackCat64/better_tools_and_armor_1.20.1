...
public class CrystalliteBowNetherDiamondImpactExplodeProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z) {
		if (!(((Entity) world.getEntitiesOfClass(Arrow.class, AABB.ofSize(new Vec3(x, y, z), 80, 80, 80), e -> true).stream().sorted(new Object() {
							...
							}.getValue()).length() > 0) {
								{
									...
									if (!_ent.level().isClientSide() && _ent.getServer() != null) {
										_ent.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, _ent.position(), _ent.getRotationVector(),
												...
												}.getValue()) + "\",Particle:\"minecraft:entity_effect\",Radius:3.0f,RadiusOnUse:-0.5f,RadiusPerTick:-0.005f,ReapplicationDelay:20,WaitTime:10}"));
									}
								}
							}
