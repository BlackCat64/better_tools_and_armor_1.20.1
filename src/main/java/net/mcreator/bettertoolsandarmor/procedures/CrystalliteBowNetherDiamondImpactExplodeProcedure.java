		...
		if (!(((Entity) world.getEntitiesOfClass(Arrow.class, AABB.ofSize(new Vec3(x, y, z), 128, 128, 128), e -> true).stream().sorted(new Object() {
		...
		}.compareDistOf(x, y, z)).findFirst().orElse(null)) == null)) {
			{
				...
				for (Entity entityiterator : _entfound) {
					if (entityiterator.getPersistentData().getBoolean("crystallite_nether_diamond_upgrade") && GetEntityLogicDataProcedure.execute(entityiterator, "inGround")) {
						if (!entityiterator.isInWaterRainOrBubble()) {
							...
							potion = GetEntityTextDataProcedure.execute(entityiterator, "Potion");
							if ((potion).length() > 0) {
								{
									Entity _ent = entityiterator;
									if (!_ent.level().isClientSide() && _ent.getServer() != null) {
